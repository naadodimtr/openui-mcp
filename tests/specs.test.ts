...
const TEST_SPEC_DIR = resolve(import.meta.dir, "..", ".test-specs");
...
tbl = Table([Col("Page", ["/home", "/pricing", "/docs", "/blog", "/about"]), Col("Views", [45000, 32000, 28000, 21000, 15000], "number"), Col("Bounce Rate", ["32%", "45%", "28%", "52%", "38%"])])`;
  ...
  it("kitchen sink - maximum complexity", async () => {
    const spec = `$showCreateModal = false
root = Stack([nav, main])
nav = Card([Stack([logo, navBtns], "row", "l", "center", "between")], "sunk")
logo = TextContent("**Enterprise App**", "large-heavy")
navBtns = Buttons([Button("Dashboard"), Button("Analytics"), Button("Settings")])
main = Stack([topBar, dashboard])
...
leftChart = Card([CardHeader("Revenue Trend"), LineChart(["Jan", "Feb", "Mar", "Apr", "May", "Jun"], [Series("2024", [180, 200, 220, 210, 250, 280]), Series("2025", [220, 250, 270, 260, 300, 340])], "natural")])
...
tableSection = Card([CardHeader("Recent Orders"), ordersTable])
ordersTable = Table([Col("Order ID", ["ORD-001", "ORD-002", "ORD-003", "ORD-004", "ORD-005"]), Col("Customer", ["Alice Johnson", "Bob Smith", "Charlie Brown", "Diana Prince", "Eve Wilson"]), Col("Amount", [299, 549, 129, 899, 449], "number"), Col("Status", ["Delivered", "Shipped", "Pending", "Delivered", "Shipped"])])`;
    await writeAndVerifySpec(spec);
